...
int init_yolov8_model(const char *model_path, rknn_app_context_t *app_ctx) {
    ...
    app_ctx->input_native_attrs.assign(input_native_attrs, input_native_attrs + io_num.n_input);
    app_ctx->output_native_attrs.assign(output_native_attrs, output_native_attrs + io_num.n_output);
...
int release_yolov8_model(rknn_app_context_t *app_ctx) {
    ...
    app_ctx->input_native_attrs.clear();
    app_ctx->output_native_attrs.clear();
...
int inference_yolov8_model(rknn_app_context_t *app_ctx, image_buffer_t *img, object_detect_result_list *od_results) {
    ...
    int bg_color = 114;
    std::vector<std::vector<int8_t>> output_bufs(app_ctx->io_num.n_output);
    ...
    for (uint32_t i = 0; i < app_ctx->io_num.n_output; i++) {
        ...
        if (app_ctx->is_quant) {
            ...
            output_bufs[i].resize(outputs[i].size);
            outputs[i].buf = output_bufs[i].data();
