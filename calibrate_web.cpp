...
bool calibrate(string& error_msg, const string& cam1_id, const string& cam2_id, bool fix_intrinsics = false) {
    lock_guard<mutex> lock(data_mutex);
    ...
    // НОВОЕ: Загрузка существующих внутренних параметров если fix_intrinsics=true
    if (fix_intrinsics) {
        string yml1 = "results/cam_" + cam1_id + "_calibration.yml";
        string yml2 = "results/cam_" + cam2_id + "_calibration.yml";
        
        FileStorage fs1(yml1, FileStorage::READ);
        FileStorage fs2(yml2, FileStorage::READ);
        ...
        if (fs1.isOpened() && fs2.isOpened()) {
            ...
            if (!K1.empty() && !D1.empty() && !K2.empty() && !D2.empty()) {
                ...
                cout << "Using existing intrinsics (K, D fixed)" << endl;
            } else {
                cout << "Existing intrinsics invalid, will calibrate from scratch" << endl;
            }
        } else {
            cout << "Intrinsics files not found, will calibrate from scratch" << endl;
        }
    }
    
    double rms = stereoCalibrate(
        object_points, image_points_1, image_points_2,
        K1, D1, K2, D2, img_size, R, T, E, F, flags,  // ← используем flags
...
// Моно-калибратор (одна камера)
class MonoCalibrator {
public:
    MonoCalibrator(const string& dev, const CalibConfig& cfg)
    ...
    bool startCamera() {
        int cam_index = -1;
        if (device.find("/dev/video") != string::npos) {
            cam_index = stoi(device.substr(device.find("video") + 5));
        }
        
        if (cam_index < 0) return false;
        ...
        if ((int)cap.get(CAP_PROP_FRAME_WIDTH) != 800) {
            cap.set(CAP_PROP_FRAME_WIDTH, 640);
            cap.set(CAP_PROP_FRAME_HEIGHT, 480);
        }
        
        actual_img_size = Size((int)cap.get(CAP_PROP_FRAME_WIDTH),
    ...
    bool getFrame(Mat& frame, string& hint_text, int& progress_current, int& progress_max) {
        if (!is_running) return false;
        
        bool ok = cap.read(frame);
        if (!ok || frame.empty()) {
            frame = Mat::zeros(480, 640, CV_8UC3);
            putText(frame, "Waiting...", Point(250, 240), FONT_HERSHEY_SIMPLEX, 1, Scalar(0,255,255), 2);
        ...
        bool found = findChessboardCorners(frame, Size(config.pattern_cols, config.pattern_rows), corners,
            CALIB_CB_ADAPTIVE_THRESH | CALIB_CB_NORMALIZE_IMAGE | CALIB_CB_FAST_CHECK);
        ...
        if (found) {
            ...
            cornerSubPix(gray, corners, Size(11,11), Size(-1,-1),
                TermCriteria(TermCriteria::EPS + TermCriteria::MAX_ITER, 30, 0.001));
            drawChessboardCorners(frame, Size(config.pattern_cols, config.pattern_rows), corners, true);
            ...
            time_t elapsed = time(nullptr) - hold_start_time;
            if (elapsed < hold_duration) {
                hint_text = "HOLD STILL... " + to_string((int)(hold_duration - elapsed)) + "s";
            } else {
                lock_guard<mutex> lock(data_mutex);
                object_points.push_back(objp);
                ...
                cout << "Frame captured: " << image_points.size() << "/" << config.max_frames << endl;
                hint_text = "CAPTURED!";
                
                if (image_points.size() >= config.max_frames) {
                    is_calibrating = false;
                }
            }
        } else {
            hold_start_time = 0;
            hint_text = "Searching for chessboard...";
        }
        
        lock_guard<mutex> lock(data_mutex);
        progress_current = image_points.size();
        progress_max = config.max_frames;
        string progress_str = to_string(progress_current) + "/" + to_string(progress_max);
        putText(frame, progress_str, Point(10, 40), FONT_HERSHEY_SIMPLEX, 0.7, Scalar(255,255,0), 2);
        
        return true;
    }
    
    bool calibrate(string& error_msg, const string& cam_id) {
        lock_guard<mutex> lock(data_mutex);
        
        if (image_points.size() < config.min_frames) {
            error_msg = "Not enough frames: " + to_string(image_points.size());
            return false;
        }
        ...
        double rms = calibrateCamera(object_points, image_points, actual_img_size,
                                     K, D, rvecs, tvecs, 0);
        
        cout << "Mono calibration RMS error: " << rms << endl;
        
        // Вычислить per-view ошибки
        vector<double> errors;
        for (size_t i = 0; i < object_points.size(); i++) {
            vector<Point2f> proj;
            projectPoints(object_points[i], rvecs[i], tvecs[i], K, D, proj);
            
            double err = 0;
            for (size_t j = 0; j < proj.size(); j++) {
                err += norm(image_points[i][j] - proj[j]);
            }
            errors.push_back(err / proj.size());
        }
        
        system("mkdir -p results");
        saveMonoCalibrationYML(cam_id, K, D, image_points.size(), actual_img_size, rms, errors, "results");
        
        vector<string> cam_ids = {cam_id};
        saveCalibrationResultsJSON(cam_ids, "results");
        
        error_msg = "RMS: " + to_string(rms);
        return true;
    }
...
"        select, input[type=number] { padding: 10px; margin: 5px; border-radius: 6px; background: #21262d; color: #c9d1d9; border: 1px solid #30363d; width: 200px; }\n"
...
"        const isStereo = calibrationMode === 'stereo';\n"
"        document.getElementById('cam2_group').style.display = isStereo ? 'block' : 'none';\n"
"    }\n"
"    \n"
...
"        const cam2 = calibrationMode === 'stereo' ? document.getElementById('cam2').value : cam1;\n"
...
"        targetFrames = parseInt(document.getElementById('max_frames').value);\n"
"        fetch(`/start?cam1=${cam1}&cam2=${cam2}&cols=${cols}&rows=${rows}&square=${square}&frames=${targetFrames}&mode=${calibrationMode}`)\n"
...
"        isCalibrating = true;\n"
"        currentFrames = 0;\n"
"        fetch('/calibrate').then(r => r.text()).then(txt => {\n"
"            if (txt === 'OK') {\n"
...
"    \n"
"    function compute() {\n"
"        document.getElementById('status').textContent = 'Computing calibration... Please wait';\n"
"        fetch('/compute')\n"
"            .then(r => r.text())\n"
"            .then(txt => {\n"
"                alert(txt);\n"
...
"            currentFrames = data.current;\n"
"            const statusText = `Status: ${data.hint} | Frames: ${data.current}/${data.max}`;\n"
"            document.getElementById('status').textContent = statusText;\n"
"            \n"
"            if (isCalibrating && currentFrames > 0 && currentFrames >= targetFrames) {\n"
"                isCalibrating = false;\n"
"                setTimeout(() => {\n"
"                    alert('Target frames reached. Starting computation...');\n"
"                    compute();\n"
"                }, 500);\n"
"            }\n"
"        });\n"
...
"                <label><input type=\"radio\" name=\"mode\" value=\"stereo\" checked onchange=\"setMode('stereo')\"> Stereo (2 cameras)</label>\n"
"                <label><input type=\"radio\" name=\"mode\" value=\"mono\" onchange=\"setMode('mono')\"> Mono (1 camera)</label>\n"
"            </div>\n"
"            \n"
...
"            <button class=\"btn-primary\" onclick=\"compute()\">Compute (Manual)</button>\n"
"        </div>\n"
"        \n"
...
//    }
    ...
    CalibConfig config;
    StereoCalibrator* stereo_calibrator = nullptr;
    MonoCalibrator* mono_calibrator = nullptr;
    string calibration_mode = "stereo";
...
svr.Get("/start", [&](const httplib::Request& req, httplib::Response& res) {
    if (req.has_param("mode")) calibration_mode = req.get_param_value("mode");
    ...
    // Очистить старые калибраторы
    if (stereo_calibrator) { stereo_calibrator->stopCameras(); delete stereo_calibrator; stereo_calibrator = nullptr; }
    if (mono_calibrator) { mono_calibrator->stopCamera(); delete mono_calibrator; mono_calibrator = nullptr; }
    
    const auto& cam1 = available_cameras[selected_cam1_idx];
    
    if (calibration_mode == "mono") {
        mono_calibrator = new MonoCalibrator(cam1.device, config);
        bool ok = mono_calibrator->startCamera();
        res.set_content(ok ? "OK" : "FAILED", "text/plain");
    } else {
        const auto& cam2 = available_cameras[selected_cam2_idx];
        stereo_calibrator = new StereoCalibrator(cam1.device, cam2.device, config);
        bool ok = stereo_calibrator->startCameras();
        res.set_content(ok ? "OK" : "FAILED", "text/plain");
    }
});
...
svr.Get("/stop", [&](const httplib::Request&, httplib::Response& res) {
    if (stereo_calibrator) stereo_calibrator->stopCameras();
    if (mono_calibrator) mono_calibrator->stopCamera();
    res.set_content("OK", "text/plain");
});

    // Start calibration
svr.Get("/calibrate", [&](const httplib::Request&, httplib::Response& res) {
    if (calibration_mode == "mono" && mono_calibrator) {
        mono_calibrator->startCalibration();
        res.set_content("OK", "text/plain");
    } else if (calibration_mode == "stereo" && stereo_calibrator) {
        stereo_calibrator->startCalibration();
        res.set_content("OK", "text/plain");
    } else {
...
svr.Get("/compute", [&](const httplib::Request&, httplib::Response& res) {
    ...
    if (calibration_mode == "mono" && mono_calibrator) {
        const auto& cam1 = available_cameras[selected_cam1_idx];
        ok = mono_calibrator->calibrate(error_msg, cam1.id);
    } else if (calibration_mode == "stereo" && stereo_calibrator) {
        const auto& cam1 = available_cameras[selected_cam1_idx];
        const auto& cam2 = available_cameras[selected_cam2_idx];
        ok = stereo_calibrator->calibrate(error_msg, cam1.id, cam2.id);
    } else {
        res.set_content("NO_CAMERAS", "text/plain");
        return;
    }
...
svr.Get("/status", [&](const httplib::Request&, httplib::Response& res) {
    ...
    if (calibration_mode == "mono" && mono_calibrator) {
        lock_guard<mutex> lock(mono_calibrator->data_mutex);
        current = mono_calibrator->image_points.size();
    } else if (calibration_mode == "stereo" && stereo_calibrator) {
        lock_guard<mutex> lock(stereo_calibrator->data_mutex);
        current = stereo_calibrator->image_points_1.size();
    }
...
svr.Get("/video", [&](const httplib::Request&, httplib::Response& res) {
    res.set_content_provider(
        ...
        [&](size_t, httplib::DataSink& sink) {
            while ((calibration_mode == "mono" && mono_calibrator && mono_calibrator->is_running) ||
                   (calibration_mode == "stereo" && stereo_calibrator && stereo_calibrator->is_running)) {
                Mat frame;
                string hint;
                ...
                if (calibration_mode == "mono" && mono_calibrator) {
                    got_frame = mono_calibrator->getFrame(frame, hint, progress_cur, progress_max);
                } else if (calibration_mode == "stereo" && stereo_calibrator) {
                    got_frame = stereo_calibrator->getFrame(frame, hint, progress_cur, progress_max);
                }
    ...
    if (stereo_calibrator) delete stereo_calibrator;
    if (mono_calibrator) delete mono_calibrator;
    return 0;
}